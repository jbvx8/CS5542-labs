...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("airplane", "car", "cat", "horse", "person", "train")
  ...
  def generateDecisonTreeModel(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(IPSettings.DECISION_TREE_PATH))) {
      println(s"${IPSettings.DECISION_TREE_PATH} exists, skipping Random Forest model formation..")
      return
    }
    ...
    println("Decison Tree Model generated")
  }
